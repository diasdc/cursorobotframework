*** Settings ***
Documentation        Aqui estarão presentes todas as Keywords responsáveis pelos testes de API.
...
Dado que eu tenha o endpoint de consulta de livros
    Log        https://fakerestapi.azurewebsites.net/api/v1
...
Quando executar a requisição de cadastro de novo livro com as informações corretas
    ${RESPOSTA}            POST On Session        booksAPI        Books/${ID_LIVRO}       expected_status=any
    ...                    json=
    ...                    headers=
